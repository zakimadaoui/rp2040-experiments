...
fn main1() -> ! {
    ...
    println!(
        "shared sram4 data value before concurrent access is {}, core1 will write it to be 7 later",
        unsafe { ptr_sram4.read_volatile() }
    );
...
unsafe fn TIMER_IRQ_0() {
    // 140 cycles delay needed to match with TIMER1 interupt
    core::arch::asm!(
    ...
    let a = {
        ...
        let end: u32;
        core::arch::asm!(
            "nop", // compensate additional instructions in TIMER1 ISR
            "nop", // compensate additional instructions in TIMER1 ISR
            "nop", // compensate additional instructions in TIMER1 ISR
...
unsafe fn TIMER_IRQ_1() {
    ...
    println!(
        "concurrent read on core 1 took {} clock cycles. written val is {}",
        a,
        ptr_sram4.read_volatile()
    );
    // stop this triggering interrupt
    unsafe { (0x4005403c as *mut u32).write(0) }; /* TIMER->INTF = 0*/
}
