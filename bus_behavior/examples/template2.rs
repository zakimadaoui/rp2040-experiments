...
fn main1() -> ! {
    ...
    pac.BUSCTRL.perfsel1.reset();
    pac.BUSCTRL.perfsel2.reset();
    pac.BUSCTRL.perfsel3.reset();
    pac.BUSCTRL
        .perfsel0
        .write(|w| w.perfsel0().sram4_contested()); // select counter 0 measurement
    pac.BUSCTRL
        .perfsel1
        .write(|w| w.perfsel1().sram5_contested()); // select counter 1 measurement
    pac.BUSCTRL
        .perfsel2
        .write(|w| w.perfsel2().sram2_contested()); // select counter 1 measurement
    pac.BUSCTRL
        .perfsel3
        .write(|w| w.perfsel3().sram3_contested()); // select counter 1 measurement
    pac.BUSCTRL.perfctr0.reset(); // reset counter
    pac.BUSCTRL.perfctr1.reset(); // reset counter
    pac.BUSCTRL.perfctr2.reset(); // reset counter
    pac.BUSCTRL.perfctr3.reset(); // reset counter
    ...
    println!(
        "contested RAM accesses [sram2 = {}, sram3= {}, sram4 = {}, sram5= {}]",
        pac.BUSCTRL.perfctr2.read().bits(),
        pac.BUSCTRL.perfctr3.read().bits(),
        pac.BUSCTRL.perfctr0.read().bits(),
        pac.BUSCTRL.perfctr1.read().bits()
    );
    pac.BUSCTRL.perfctr0.reset(); // reset counter
    pac.BUSCTRL.perfctr1.reset(); // reset counter
    pac.BUSCTRL.perfctr2.reset(); // reset counter
